...
# # import number_reccognition

# # number_reccognition.cv2.imshow("ImageWhite",number_reccognition.imgWhite)
...
#     # process = os.popen("python number_recognition.py")  # Use popen for output capture
#     # index = process.read().strip()  # Read and strip output
#     # return render_template('number.html', prediction=index), 200

#     os.system("python char_recognition.py")  
#     return 'Video saved and processed successfully!', 200  
...
def process_video():
    ...
    if 'video' in request.files:
        ...
        try:
            # Execute the command and capture the output
            result = subprocess.run(["python", "pose.py"], capture_output=True, text=True, check=True)
            temp2 = result.stdout.strip()
            print("Temp2 is", temp2)
